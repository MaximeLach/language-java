...
public class Morpion {
    ...
    public static boolean verifierGagnant(char tab[][]) {
     ...
     public static boolean tableauPlein(char tab[][]) {
        boolean plein = true;
        ...
        for (int ligne = 0; ligne <3;ligne++) {
            for(int colonne = 0;colonne <3;colonne++)
                if(tab[ligne][colonne] == ' ') plein = false;    
            }
     return plein;   
     }
    ...
    do{
        ...
        do {
            ...
            afficherTableau(tab);
        } while (verifierGagnant(tab) == false && tableauPlein(tab) == false);
        if (verifierGagnant(tab) == true )System.out.println("Joueur " + joueur + " a gagné !!!");
        else System.out.println("Partie terminées, match nul !!!");
    }
}
